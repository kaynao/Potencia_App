...
function renderPainelEntregas() {
  ...
  if (entregasPendentes.length === 0) {
    ...
    entregasPendentes.slice().reverse().forEach(v => {
  ...
  if (entregasConcluidas.length === 0) {
    ...
    entregasConcluidas.slice().reverse().forEach(v => {
...
function exportarParaExcel(dados, nomeArquivo = 'relatorio.xlsx') {
  ...
  // Cria uma nova planilha a partir do array de objetos JavaScript
  const worksheet = XLSX.utils.json_to_sheet(dados);
  const workbook = XLSX.utils.book_new();
  
  XLSX.utils.book_append_sheet(workbook, worksheet, "Relatório");

  // Baixa o arquivo diretamente como .xlsx
  XLSX.writeFile(workbook, nomeArquivo);
}

// Exemplo de uso para exportar as Vendas:
...
// Exemplo de uso para exportar o Estoque:
